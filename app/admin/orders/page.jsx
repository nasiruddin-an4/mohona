...
export default function OrdersPage() {
  ...
  const getRStatusStyle = (status) => {
    switch(status?.toLowerCase()) {
      ...
      case 'processing': return 'bg-[#fef3c7] text-amber-700';
              ...
              <tbody className="divide-y divide-gray-50">
                {orders.length > 0 ? (
                  orders.map((order, index) => {
                    const itemCount = order.order_items?.length || Math.floor(Math.random() * 10) + 1; // Fallback if no items array
                    ...
                    return (
                      <tr key={order._id || index} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-4 py-4 text-gray-500 font-medium">#{order._id?.slice(-5).toUpperCase()}</td>
                        ...
                        <td className="px-4 py-4 font-bold text-gray-900">${order.total_amount?.toFixed(2) || '0.00'}</td>
                        <td className="px-4 py-4 text-center">
                          <span className={`inline-flex px-3 py-1 text-[11px] font-bold rounded-full border ${getPStatusStyle(isPaid)} uppercase tracking-wider`}>
                            {isPaid}
                          </span>
                        </td>
                        <td className="px-4 py-4 text-center">
                          <span className={`inline-flex px-3 py-1 text-[11px] font-bold rounded-md ${getRStatusStyle(recStatus)}`}>
                            {recStatus}
                          </span>
                        </td>
