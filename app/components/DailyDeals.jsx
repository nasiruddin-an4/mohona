...
export default function DailyDeals({ products = [], loading = false }) {
  const scrollRef = useRef(null);
  const [isPaused, setIsPaused] = useState(false);
  const addItem = useCartStore((state) => state.addItem);

  // Filter products that have discounts
  const dealProducts = products.filter(p => p.discount_pct && p.discount_pct > 0).slice(0, 15);
  ...
  // Autoplay Logic
  useEffect(() => {
    if (isPaused || dealProducts.length === 0) return;
  ...
  }, [scroll, isPaused, dealProducts.length]);
    ...
    >
      ...
      {dealProducts.length > 5 && (
        <>
          <button 
            onClick={() => scroll('left')}
            className="absolute left-0 top-[55%] -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#f18e6c] hover:scale-110 transition-all opacity-0 group-hover:opacity-100 -translate-x-5"
          >
            <ChevronLeft size={20} strokeWidth={3} />
          </button>
          <button 
            onClick={() => scroll('right')}
            className="absolute right-0 top-[55%] -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#f18e6c] hover:scale-110 transition-all opacity-0 group-hover:opacity-100 translate-x-5"
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-gray-500">No deals available today.</div>
      )}
    </section>
  );
}
