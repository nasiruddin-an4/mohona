...
// Removed hardcoded shopLocations
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const fetchOutlets = async () => {
      try {
        const response = await fetch('/api/public-outlets');
        const data = await response.json();
  ...
  // Close shop dropdown, mobile menu and mobile search when navigating
  useEffect(() => {
  ...
  return (
    <>
      ...
      <nav className="bg-gray-50 sticky top-0 z-50 py-4 shadow-md">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-8">
              ...
              <Link href="/" className="flex items-center gap-2 group mr-4">
                <img src="/logoFinal.jpg" alt={settings?.storeName || "Mohona by CGFWA"} className="h-11 sm:h-12 md:h-16 w-auto object-contain py-1" />
              </Link>
              ...
              <div className="hidden lg:flex items-center gap-6 text-[15px] font-semibold text-gray-700">
                ...
                {/* Shop with Dropdown */}
                  ...
                  >
                    <button onClick={() => setShopMenuOpen(!shopMenuOpen)} className="flex items-center gap-1 hover:text-black transition-colors focus:outline-none">
                      <span>Shop</span>
                      <ChevronDown
                        size={14}
            ...
            <div className="flex items-center gap-5">
              ...
              {/* Mobile Search Toggle (phones only — the bar above is hidden below md) */}
              <button
              ...
              {/* Mobile Menu Toggle — visible through tablet widths too, since the inline
                  Shop/About/Contact menu only appears at the lg breakpoint */}
              <button
        ...
        {isMenuOpen && (
          <div
            className="lg:hidden fixed inset-0 bg-black/50 z-[60] transition-opacity animate-in fade-in duration-300"
            onClick={() => setIsMenuOpen(false)}
          />
        )}

        {/* Mobile Menu */}
        <div className={`lg:hidden fixed top-0 right-0 h-full w-[80%] max-w-sm bg-white shadow-2xl z-[70] transform transition-transform duration-300 ease-in-out flex flex-col ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
